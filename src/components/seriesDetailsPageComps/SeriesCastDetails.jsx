...
function SeriesCastDetails() {
  const { castSeries, actorSeriesLoading, actorSeriesError } = useSelector(
    (state) => state.ActorForSeriesSlice
  );
  ...
  return (
    <div className="second-mov-or-ser-section px-14 pt-10 ">
      ...
      <div
        ...
        style={{ overflowX: castSeries.length > 4 && "scroll" }}
      >
        <div
          style={{
            width: `${shadowNum}px`,
              ...
              castSeries.map((ser) => (
                ...
                >
                  <div className="every-person-in-cast-img ">
                    <img
                      src={
                        ser.profile_path !== null
                          ? `https://image.tmdb.org/t/p/w600_and_h900_bestv2${ser.profile_path}`
                          : img
                      }
                  ...
                  <div className="every-person-in-cast-info capitalize  text-white pt-2 pl-3">
                    <h2 className="h-14 text-xl mb-2">{ser.original_name}</h2>
                    <h2 className=" text-base">{ser.character}</h2>
                  </div>
                </div>
              ))
            ) : (
